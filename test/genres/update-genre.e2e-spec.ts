...
import { UpdateGenreFixture } from '@/modules/genres-module/testing/genre-fixture';
...
describe('GenresController (e2e)', () => {
  ...
  describe('/genres/:id (PATCH)', () => {
    ...
    describe('should a response error with 422 when throw EntityValidationError', () => {
      const app = startApp();
      const validationErrors = UpdateGenreFixture.arrangeForEntityValidationError();
    ...
    describe('should update a genre', () => {
      ...
      test.each(arrange)('when body is $send_data', async ({ send_data, expected, relations }) => {
        ...
        const genreCreated = Genre.fake().aGenre().addCategoryId(category.id).build();
        await genreRepo.insert(genreCreated);

        const res = await request(app.app.getHttpServer())
          .patch(`/genres/${genreCreated.id.value}`)
          .send(send_data)
          .expect(200);
        ...
        const presenter = GenresController.serialize(GenreOutputMapper.toOutput(genreUpdated!, relations.categories));
