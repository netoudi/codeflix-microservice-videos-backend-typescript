...
describe('RabbitmqConsumeErrorFilter Unit Tests', () => {
  ...
  beforeEach(async () => {
    const module: TestingModule = await Test.createTestingModule({
      providers: [RabbitmqConsumeErrorFilter, { provide: AmqpConnection, useValue: { publish: jest.fn() } }],
    }).compile();
    await module.init();
  ...
  describe('catch', () => {
    ...
    it('should retry if error is retriable and retry count is less than max retries', async () => {
      ...
      expect(host.getType).toHaveBeenCalled();
      expect(amqpConnection.publish).toHaveBeenCalledWith('direct.delayed', 'test', Buffer.from('test'), {
