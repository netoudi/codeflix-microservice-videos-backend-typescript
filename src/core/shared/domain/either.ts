...
export class Either<Ok = unknown, ErrorType = Error> implements Iterable<Value<Ok, ErrorType>> {
  ...
  static safe<Ok = unknown, ErrorType = Error>(fn: () => Ok): Either<Ok, ErrorType> {
    try {
      return Either.ok(fn());
    } catch (e) {
  ...
  static fail<ErrorType = Error, Ok = unknown>(error: ErrorType): Either<Ok, ErrorType> {
   ...
   */
  chain<NewOk, NewError = Error>(fn: (value: Ok) => Either<NewOk, NewError>): Either<NewOk, ErrorType | NewError> {
    if (this.isOk()) {
      return fn(this.ok);
    }
    return Either.fail(this.error);
...
class EitherIterator<Ok, Error> implements Iterator<Value<Ok, Error>, Value<Ok, Error>, undefined> {
