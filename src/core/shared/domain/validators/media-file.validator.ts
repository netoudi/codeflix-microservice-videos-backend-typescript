...
export class MediaFileValidator {
  ...
  validate({ raw_name, mime_type, size }: { raw_name: string; mime_type: string; size: number }) {
...
export class InvalidMediaFileMimeTypeError extends Error {
  constructor(actual_mime_type: string, valid_mime_types: string[]) {
    super(`Invalid media file mime type: ${actual_mime_type} not in ${valid_mime_types.join(', ')}`);
  }
}
