...
export class VideoSequelizeRepository implements IVideoRepository {
  ...
  async existsById(ids: VideoId[]): Promise<{ exists: VideoId[]; not_exists: VideoId[] }> {
    ...
    const existsVideoIds = existsVideoModels.map((m) => new VideoId(m.id));
  ...
  async update(entity: Video): Promise<void> {
    ...
    await Promise.all([
      ...model.image_medias.map((i) => i.destroy({ transaction: this.uow.getTransaction() })),
    ...
    const { categories_id, genres_id, cast_members_id, image_medias, audio_video_medias, ...props } =
  ...
  async delete(id: VideoId): Promise<void> {
    const videoCategoryRelation = this.videoModel.associations.categories_id.target;
    const videoGenreRelation = this.videoModel.associations.genres_id.target;
    const videoCastMemberRelation = this.videoModel.associations.cast_members_id.target;
    const imageMediaModel = this.videoModel.associations.image_medias.target;
    const audioVideoMediaModel = this.videoModel.associations.audio_video_medias.target;
  ...
  async search(props: VideoSearchParams): Promise<VideoSearchResult> {
    ...
    if (
      props.filter &&
      (props.filter.title || props.filter.categories_id || props.filter.genres_id || props.filter.cast_members_id)
    ) {
