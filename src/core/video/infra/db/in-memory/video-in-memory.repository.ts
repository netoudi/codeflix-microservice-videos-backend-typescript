...
import { VideoFilter, IVideoRepository } from '@/core/video/domain/video.repository';
  ...
  implements IVideoRepository
{
  ...
  protected async applyFilter(items: Video[], filter: VideoFilter | null): Promise<Video[]> {
    if (!filter) {
      return items;
    }
    return items.filter((i) => {
      const containsTitle = filter.title && i.title.toLowerCase().includes(filter.title.toLowerCase());
      const containsCategoriesId =
        filter.categories_id && filter.categories_id.some((c) => i.categories_id.has(c.value));
      const containsGenresId = filter.genres_id && filter.genres_id.some((c) => i.genres_id.has(c.value));
      const containsCastMembersId =
        filter.cast_members_id && filter.cast_members_id.some((c) => i.cast_members_id.has(c.value));
      const filterMap = [
  ...
  protected applySort(items: Video[], sort: string | null, sort_dir: SortDirection | null): Video[] {
