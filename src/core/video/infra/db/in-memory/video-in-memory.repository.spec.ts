...
describe('VideoInMemoryRepository', () => {
  ...
  it('should no filter items when filter object is null', async () => {
    const items = [Video.fake().aVideoWithoutMedias().build(), Video.fake().aVideoWithoutMedias().build()];
  ...
  it('should filter items by title', async () => {
    ...
    const items = [faker.withTitle('test').build(), faker.withTitle('TEST').build(), faker.withTitle('fake').build()];
    const filterSpy = jest.spyOn(items, 'filter' as any);
    const itemsFiltered = await repository['applyFilter'](items, { title: 'TEST' });
  ...
  it('should filter items by categories_id', async () => {
    ...
    const items = [
      Video.fake().aVideoWithoutMedias().addCategoryId(categoryId1).addCategoryId(categoryId2).build(),
      Video.fake().aVideoWithoutMedias().addCategoryId(categoryId3).addCategoryId(categoryId4).build(),
    ];
    const filterSpy = jest.spyOn(items, 'filter' as any);
  ...
  it('should filter items by title and categories_id', async () => {
    ...
    const items = [
      ...
      Video.fake()
        ...
        .build(),
      Video.fake().aVideoWithoutMedias().withTitle('test fake').addCategoryId(categoryId1).build(),
    ];
  ...
  it('should filter items by title and categories_id and genres_id', async () => {
    ...
    const items = [
      ...
      Video.fake()
        ...
        .addCategoryId(categoryId4)
        .build(),
      Video.fake().aVideoWithoutMedias().withTitle('test fake').addCategoryId(categoryId1).addGenreId(genreId1).build(),
    ];
