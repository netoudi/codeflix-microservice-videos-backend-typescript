...
export type VideoSearchParamsCreateCommand = Omit<SearchParamsConstructor<VideoFilter>, 'filter'> & {
  filter?: {
    title?: string;
...
export class VideoSearchParams extends SearchParams<VideoFilter> {
  private constructor(props: SearchParamsConstructor<VideoFilter> = {}) {
    super(props);
  }

  static create(props: VideoSearchParamsCreateCommand = {}) {
    const categories_id = props.filter?.categories_id?.map((c) => (c instanceof CategoryId ? c : new CategoryId(c)));
    const genres_id = props.filter?.genres_id?.map((c) => (c instanceof GenreId ? c : new GenreId(c)));
...
export interface IVideoRepository
  extends ISearchableRepository<Video, VideoId, VideoFilter, VideoSearchParams, VideoSearchResult> {}
