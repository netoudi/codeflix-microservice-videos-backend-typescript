...
export class VideoFakeBuilder<TBuild = any> {
  ...
  private _thumbnail: PropOrFactory<Thumbnail | null> | undefined = new Thumbnail({
    name: 'test-name-thumbnail.png',
    location: 'test path thumbnail',
  });
  private _thumbnail_half: PropOrFactory<ThumbnailHalf | null> | undefined = new ThumbnailHalf({
    name: 'test-name-thumbnail-half.png',
    location: 'test path thumbnail half',
  });
  private _trailer: PropOrFactory<TrailerMedia | null> | undefined = TrailerMedia.create({
    name: 'test-name-trailer.mp4',
    raw_location: 'test path trailer',
  });
  private _video: PropOrFactory<VideoMedia | null> | undefined = VideoMedia.create({
  ...
  withTrailer(valueOrFactory: PropOrFactory<TrailerMedia | null>) {
    this._trailer = valueOrFactory;
    return this;
  }

  withTrailerComplete() {
    this._trailer = TrailerMedia.create({
  ...
  build(): TBuild {
    const videos = new Array(this.countObjs).fill(undefined).map((_, index) => {
      ...
      const categoriesId = this._categories_id.length ? this.callFactory(this._categories_id, index) : [categoryId];

      const genreId = new GenreId();
      const genresId = this._genres_id.length ? this.callFactory(this._genres_id, index) : [genreId];
      ...
      video['id'] = !this._video_id ? video['id'] : this.callFactory(this._video_id, index);
      video.validate();
      return video;
    });
