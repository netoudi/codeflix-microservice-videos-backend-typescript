  ...
  implements IGenreRepository
{
  ...
  protected async applyFilter(items: Genre[], filter: GenreFilter): Promise<Genre[]> {
    ...
    return items.filter((genre) => {
      const containsName = filter.name && genre.name.toLowerCase().includes(filter.name.toLowerCase());
      const containsCategoriesId =
        filter.categories_id && filter.categories_id.some((c) => genre.categories_id.has(c.value));
